...
def generate_models(x, y, degs):
    ...
    models = []
    for d in degs:
        # generate model for every degree
        # tuple with polynomial variable values
        # note numpy.polyfit == pylab.polyfit
...
#print(r_squared([1, 2, 3], [1, 2, 3.2352]))
# Problem 3
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    # generate figure for models
    for mod in models:
        pylab.plot(x, y, "bo")
        estdata = np.polyval(mod, x)
        pylab.plot(x, estdata, "r")
        # title etc
        pylab.title("Mean Temp/Year\nCoefficients: {},\nR^2: {}".format(
            mod, r_squared(y, estdata)))
        pylab.xlabel('Year')
        pylab.ylabel('Temp')
        pylab.show()
...
if Prob3 == True:
    ...
    for year in INTERVAL_1:
        # y.append(raw_data.get_yearly_temp('LOS ANGELES', 2000)) BULLSHIT
