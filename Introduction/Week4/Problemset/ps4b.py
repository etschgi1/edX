...
def playGame(wordList):
    ...
    valid_input = ['r', 'n', 'e', 'R', 'N', 'E']
    # establish game loop
    while True:
        # ask for input either r(replay) n(new hand) e(end game)
        userin = input('Enter n to deal a new hand, r to replay the last hand,'
                       ' or e to end game: ')
        # enter if user or computer should play
        userin2 = input('Enter u to have yourself play, c'
                        ' to have the computer play:')
        if userin2 == 'c':
            # let comp play hand
            comphand = dealHand(HAND_SIZE)
            # let comp play his hand
